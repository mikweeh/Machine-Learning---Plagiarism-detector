...
def get_original_filename(df, answer_filename):
    """This functions returns the filename of the original file that is the original answer
    to the task treated in the file 'answer_filename'.
    Inputs: df is a dataframe with at least a column with 'File', 'Task' asn 'Class', and also
    it has to have the originals in rows and labeled as -1 in the class field
    Output: The filename of the original file"""
    
    actual_task = (df.loc[df['File']==answer_filename])['Task'].values[0]
    df_orig = (df.loc[df['Class']==-1])
    orig_filename = (df_orig.loc[df_orig['Task']==actual_task])['File'].values[0]
    
    return orig_filename


def get_files_indexes(df, answer_filename):
    """This functions returns an array of two elements. Both are indexes associated with
    main dataframe. The first indicates de answer file and the second the source file"""
    
    temp1 = df.loc[df['File']==answer_filename] # Returns a df with 1 row, the one of the filename
    index_a = temp1.index[0] # Returns the index of this row in main dataframe
    actual_task = temp1['Task'].values[0] # Estract the value of the task
    
    df_orig = (df.loc[df['Class']==-1]) # Return a df with the originals (nothing else)
    temp2 = df_orig.loc[df_orig['Task']==actual_task] # Return a df with 1 row, the original with that task
    index_s = temp2.index[0]
    #print(temp2['File'].values[0]) # To check that is the correct task
    
    result = [index_a, index_s]
